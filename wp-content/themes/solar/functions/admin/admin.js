...
jQuery(document).ready(function(){
    ...
    jQuery('#gallery_categories_box_meta').hide();

    var page_template_select = jQuery('#page_template');
    var templates_array = new Array('templates/template-gallery-fs.php', 'templates/template-gallery-grid.php', 'templates/template-gallery-grid-info.php');

    //Hide sidebar meta box on load if certain template selected
    var template_load = page_template_select.val();
    if(templates_array.indexOf(template_load) != -1){
        jQuery('#sidebar_position_meta').hide();
        jQuery('#gallery_categories_box_meta').show();
    }
    else {
        jQuery('#gallery_categories_box_meta').hide();
    }
    ...
    page_template_select.change(function(){
        ...
        if(templates_array.indexOf(template) != -1){
            jQuery('#sidebar_position_meta').hide();
            jQuery('#gallery_categories_box_meta').show();
        }
        else {
